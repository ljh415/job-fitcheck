...
작성 순서:
1. 이력서에서 해당 자격요건에 대한 명시적 근거(기술명·프로젝트명·역할)를 먼저 찾는다. 유사 기술이나 상위 카테고리로 대체 판정하지 않는다
2. 근거가 있으면 ✅, 없으면 ❌, 유사 경험은 있으나 정확히 일치하지 않으면 🔲
3. gaps 배열에도 반드시 동일 기준으로 반영한다. ❌ 항목은 gaps에 빠짐없이 포함
4. 비기술 요건도 별도로 확인한다: 경력 연수(요구 vs 보유), 근무지(공고 위치 vs 후보자 희망 지역), 도메인 경험 등. 불일치가 있으면 gaps에 포함
5. gaps 배열 작성 후, 자격요건 테이블의 ❌·🔲 항목 개수를 세어 gaps 배열 항목 수와 대조한다. 테이블의 ❌·🔲 항목 수보다 gaps 항목 수가 적으면 빠진 항목이 있다는 뜻이므로, 테이블을 다시 훑어 누락된 항목을 gaps에 추가한다. step 4의 비기술 갭도 이 대조에 포함한다
