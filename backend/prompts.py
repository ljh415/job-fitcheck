...
평가 기준 (우선순위 순):
1. 연봉/처우
   - 공고에 연봉이 명시되어 있고 후보자 희망 최소 연봉 이상이면 가산 요소로 반영.
   - 공고에 연봉이 미명시이거나 희망 연봉 미달이더라도 감점하지 않음 (미명시가 대부분이므로 패널티 없음).
2. 회사 규모/안정성 (임직원 50명+, 실서비스·실매출)
3. 근무 형태/위치
4. 직무 적합도
   - required_skills(자격요건): 통과/탈락 기준으로 먼저 판단. 미충족 항목은 강한 감점 요인. 충족은 기본값이므로 가산 없음.
   - preferred_skills(우대사항): 변별력 가산 요소. 충족 항목이 많을수록 점수 상승. 특히 자격요건이 평이하거나 보편적(누구나 충족 가능한 수준)일 경우, 우대사항 충족 비율이 최종 점수에 더 큰 영향을 미침.
   - key_responsibilities(주요 업무): 실제 수행 업무와 후보자 경험의 직접 연관성을 확인.
{custom_criteria}"""
