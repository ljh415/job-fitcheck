...
- **결격사유형 자격 조항**: 특별한 문제가 없으면 통상적으로 충족되는 항목(예: 해외 출장 가능 여부 등 — 직무 역량과 무관하게 지원 자체의 형식적 자격을 확인하는 항목). 이력서는 보통 이런 항목을 적극적으로 언급하지 않으므로, 명시적 언급이 없다는 이유만으로 ❌로 판정하지 마세요 — 이력서에 반대되는 명시적 증거(예: "해외출장 불가"라고 직접 쓰여 있는 경우)가 있을 때만 갭으로 잡으세요. 언급이 없으면 "평가상 충족으로 간주 — 실제 여부는 본인 확인 필요"로 근거를 남기고 gaps에 포함하지 마세요.
...
2. 자격요건 표는 근거가 있으면 ✅, 없으면 ❌ 두 가지로만 판정한다(유사 경험만으로는 ✅ 불가 — 이 경우도 ❌). 우대사항 표는 근거가 있으면 ✅, 없으면 ❌, 유사 경험은 있으나 정확히 일치하지 않으면 🔲로 판정한다. **단, 위 [자격요건 유형 구분]에서 "결격사유형 자격 조항"으로 분류된 항목은 이 기본값을 따르지 않고 그 규칙을 따른다.**
...
판정 기준 (자격요건 표): ✅ 충족 = 이력서에 명시적 근거가 있을 때만. ❌ 미충족 = 언급이 전혀 없거나 유사 기술만 있는 경우 (기본값, 단 결격사유형 조항은 예외)
