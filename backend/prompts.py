...
EXTRACT_COMPANY_TOOL_SCHEMA = {
    ...
    "properties": {
        ...
        "revenue_status": {"type": ["string", "null"], "description": "매출 현황. 구체적 수치나 설명이 있으면 그대로 기재 (예: '2023년 매출 200억', '흑자 전환', 'BEP 달성', '매출 성장 중'). 단순히 매출이 있다는 사실만 알 수 있으면 null."},
...
2. 근거가 있으면 ✅, 없으면 ❌, 유사 경험은 있으나 정확히 일치하지 않으면 🔲
...
EVALUATE_FIT_TOOL_SCHEMA = {
    ...
    "properties": {
        ...
        "strengths": {
            "type": "array",
            "items": {"type": "string"},
            "description": "후보자 강점. 형식: '(강) 항목명 - 근거 한 줄'. 강도는 항목 맨 앞에 (강)·(중)·(약) 중 하나로 표기. 각 항목은 특정 프로젝트·회사에서의 단일 경험을 서술하고, 서로 다른 회사/프로젝트의 경험을 하나로 묶지 마세요. 예: '(강) Airflow 기반 배치 파이프라인 구축 - 휴레이포지티브에서 생체신호 데이터 일 단위 전처리 파이프라인 설계'"
        },
        "gaps": {
            "type": "array",
            "items": {"type": "string"},
