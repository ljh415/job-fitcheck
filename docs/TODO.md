...
- 🔧 프로필 스냅샷 & 적합도 평가 히스토리 — 프로필 변경 시점마다 스냅샷 보관(업로드/수동편집 시 메모 남길 수 있음, 목록에서 메모 수정도 가능) + 회사별 refit 결과를 덮어쓰지 않고 이력으로 누적, 각 이력이 어느 프로필 스냅샷 기준인지 연결(예: 사이드 프로젝트 추가 후 점수가 몇 점 올랐는지 추적). SQLite(`data/app.db`)로 저장(RAG의 Postgres는 opt-in이라 의존 안 함, `usage_log` 같은 다른 후보도 같은 파일로 옮길 계획은 별도 브랜치). 세부 설계는 `docs/profile-history/PLAN.md`, 개발 히스토리는 `docs/profile-history/HISTORY.md` 참고(둘 다 로컬 전용). `feat/profile-history` 브랜치(dev worktree)에서 계획된 9단계(백엔드 6+프론트엔드 3: SQLite 모듈, 스냅샷/이력 생성 훅, 프로필 버전 목록·조회·삭제·메모 API, 회사 평가 이력 API, 기존 회사 소급 적용, 설정 화면 UI, 프로필 버전 뷰어, 회사 상세 평가 이력 토글+상세 뷰어) 전부 구현·dev 실측 검증 완료(2026-08-16). 남은 건 main merge 시점 결정.
