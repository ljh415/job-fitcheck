...
- ✅ MCP 구현·배포 완료(2026-08-26, v1.6.0) — 도구 11개(`search_rag_evidence`만 신규 함수, 나머지는 기존 REST/내부 함수 재사용) 전부 구현. Codex 리뷰 2라운드(중간 1+낮음 4, 이후 중간 1+낮음 2)와 자체 리뷰(2건)로 총 7건 버그 수정(잘못된 status 저장으로 회사 읽기 불가, source_url 중복검사 우회, 입력 필드 미강제, 최초 적합도 이력 누락, 알림 미발송, 불필요한 RAG 재색인, 값 범위 미검증, REST 입력 제한 미재사용, 도구 에러 메시지가 클라이언트에 전달 안 됨). `feat/mcp-server` 브랜치에서 main merge 후 prod 배포했다가, 실제 운영 데이터(116개 회사)에서 MCP 클라이언트 라이브러리의 1MB SSE 이벤트 제한을 초과해 `list_companies`가 실패하는 걸 발견 — 즉시 prod `git revert`로 원복, `fix/mcp-tool-call-failure` 브랜치에서 원인 규명(서버는 정상, 클라이언트 라이브러리 하드 리밋 확인)·수정(목록 응답에서 문장 단위 상세 필드 제외) 후 별도 Codex 리뷰 거쳐 재배포. 사용자 가이드는 `MCP_GUIDE.md`(MCP 등록 회사와 웹 등록 회사의 적합도 점수 일관성 문제도 알려진 제약으로 기록). 상세 기록: `docs/mcp-server/PLAN.md`·`HISTORY.md`(dev worktree, 로컬 전용), `docs/planning/mcp_sse_size_limit.md`(장애 기록, 로컬 전용)
