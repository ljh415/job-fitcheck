...
- 🔧 v7 튜닝 — 심각도 규칙 유지 + 비요건 갭(경력·근무지·평점·도메인)은 필수/우대 구분과 무관하게 반드시 포함
- ⬜ v7 회귀 검증 — 반복(n≥3) × 3케이스(NHN·딥파인·다키), 갭 탐지율·점수 Δ 집계
- ⬜ 검증 완료 후 main 승격 + prod 재빌드
...
- ⬜ prod 실데이터 오염: NHN·딥파인·다키 3사 리포트가 실험 중 Gemini(v5) 생성본으로 덮어써짐 → Claude refit 원복 필요 (사용자 승인 후 실행)
