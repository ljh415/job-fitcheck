...
- ✅ QnA 대화 기록을 서버(SQLite)에 저장하도록 전환 — 여러 기기(모바일·집 데스크탑·개인 맥북)를 오가며 쓰는데 QnA 기록이 브라우저 localStorage에만 있어 기기 간 연속성이 없던 문제(사용자 제보, 2026-08-21). `data/app.db`에 `qa_messages` 테이블 추가, pending 상태도 서버에 보존(질문 즉시 INSERT, 응답 시 UPDATE). 설계 중 핵심 발견: `navigate()`가 화면 전환 시 SSE 연결을 끊는데 LLM 호출을 요청 코루틴 안에서 직접 돌리면 페이지 이동 시 처리도 같이 끊겨 "떠났다 돌아오면 답변이 채워져 있다"는 목적이 깨짐 → `asyncio.create_task()` 기반 독립 태스크로 LLM 호출 분리, SSE는 큐 중계만 담당. 컨텍스트도 클라이언트가 보내던 방식 대신 서버가 `qa_messages`에서 직접 조회(최근 20턴, done만)하도록 변경. 좀비 pending은 시간 추측 없이 서버 시작 시점 일괄 정리로 확정적 해결. localStorage→서버 1회성 마이그레이션(`migrate-qa`)도 기기별 자동·조용히 실행되도록 구현. curl 강제 연결종료·Playwright 실제 브라우저 시나리오(질문 전송 후 화면 이동→복귀) 양쪽으로 검증 — 초반 대기시간을 짧게 잡아 "취소된 줄" 오판했다가 재확인 결과 정상 완료 확인. RAG 채팅 서버 저장은 별도 버전으로 후속 진행(`docs/chat-history-server-storage/PLAN.md`). `feat/chat-history-server-storage` 브랜치에서 `main`에 merge 완료(2026-08-22, v1.4.18).
