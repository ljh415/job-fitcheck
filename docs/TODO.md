...
- ✅ 코드 내 주석 "고고학적 서사"(누가·언제·어떤 리뷰 라운드에서 발견했는지의 조사 경위) 일괄 정리 — 30개 파일 106건, WHY(비자명한 불변조건·제약)는 남기고 발견 경위·날짜·"Codex 리뷰로 발견" 같은 인용만 제거. 정리 과정에서 CHANGELOG에 누락돼 있던 v1.5.2 항목 4건도 함께 보강. `refactor/comment-cleanup` 브랜치(17개 커밋)에서 `main`에 merge 완료(2026-08-23)
...
## Phase 11 — RAG 서브프로젝트 main 반영 + MCP 서버 구현 (RAG main 반영 완료, MCP 구현·배포 완료, 2026-08-26)
...
- ✅ MCP 구현·배포 완료(2026-08-26, v1.6.0) — 도구 11개(`search_rag_evidence`만 신규 함수, 나머지는 기존 REST/내부 함수 재사용) 전부 구현. Codex 리뷰 2라운드(중간 1+낮음 4, 이후 중간 1+낮음 2)와 자체 리뷰(2건)로 총 7건 버그 수정(잘못된 status 저장으로 회사 읽기 불가, source_url 중복검사 우회, 입력 필드 미강제, 최초 적합도 이력 누락, 알림 미발송, 불필요한 RAG 재색인, 값 범위 미검증, REST 입력 제한 미재사용, 도구 에러 메시지가 클라이언트에 전달 안 됨) — 전부 `feat/mcp-server`가 main에 처음 merge되기 전에 발견·수정. 사용자 가이드는 `MCP_GUIDE.md`(MCP 등록 회사와 웹 등록 회사의 적합도 점수 일관성 문제도 알려진 제약으로 기록). 상세 기록: `docs/mcp-server/PLAN.md`·`HISTORY.md`(dev worktree, 로컬 전용)
- ✅ MCP prod 배포 직후 장애 수정(2026-08-26, v1.6.1, `feat/mcp-server`와 별도 버전) — main merge 후 prod 배포했다가, 실제 운영 데이터(116개 회사)에서 MCP 클라이언트 라이브러리의 1MB SSE 이벤트 제한을 초과해 `list_companies`가 실패하는 걸 발견 — 즉시 prod `git revert`로 원복, `fix/mcp-tool-call-failure` 브랜치에서 원인 규명(서버는 정상, 클라이언트 라이브러리 하드 리밋 확인)·수정(목록 응답에서 문장 단위 상세 필드 제외) 후 별도 Codex 리뷰 거쳐 재배포. 상세 기록: `docs/planning/mcp_sse_size_limit.md`(로컬 전용)
- ✅ MCP `create_company` 프로필 버전 이력 버그 2건 수정(2026-08-27, v1.6.2) — CLI 실사용 QA 중 발견, `fix/mcp-profile-version-tracking` 브랜치에서 수정·Codex 리뷰·main merge 완료. 상세 기록: `docs/mcp-server/HISTORY.md`(dev worktree, 로컬 전용)
- ✅ MCP 대화맥락 격리 지시 + 적합도 평가 프롬프트(`EVALUATE_FIT_SYSTEM`, REST/MCP 공유) 일관성 개선(2026-08-28, v1.6.3) — MCP-vs-웹 점수 불일치 후속 조사, 진짜 원인은 대화 오염이 아니라 프롬프트 자체의 판단 불안정으로 확인. `feat/mcp-context-isolation`+`feat/fit-eval-prompt-consistency` main merge 완료. 상세 기록: `docs/mcp-server/HISTORY.md`, `docs/fit-eval-consistency/PLAN.md`·`HISTORY.md`(dev worktree, 로컬 전용)
- ✅ `EVALUATE_FIT_SYSTEM` 소프트스킬 결격사유형 오분류 수정 + 복합 자격요건 판정 규칙 추가(2026-08-31, v1.6.4) — 2단계 MCP 검증에서 소프트 스킬이 결격사유형 조항으로 자동 충족 처리되는 버그 발견·수정(버전 G), 이어서 입력 고정 실험에서 복합요건(AND/OR/불명확 연결어) 판정이 표·gaps·종합의견 간 내부 모순을 일으키는 버그 추가 발견·수정(버전 H). `fix/soft-skill-eligibility-misclassification` main merge 완료. 소프트스킬 수정은 REST 반복 검증에서 완전히는 안 잡히는 잔여 결함(약 1/3) 확인 — 프롬프트 문구 추가 대신 구조적 개선(판정 단계 분리 + 코드 기반 완결성 검증) 논의 진행 중. 상세 기록: `docs/fit-eval-consistency/PLAN.md`·`HISTORY.md`(dev worktree, 로컬 전용)
...
- ✅ RAG 실사용 준비 중 발견된 5건 — prod에서 RAG를 처음 켜고 실사용 화면을 캡처하려던 중 사용자가 직접 발견(2026-08-18). (1) `.env.example`의 `RAG_POSTGRES_DB`/`USER`/`PASSWORD`/`PORT`가 이미 코드 기본값과 동일한데 빈 칸처럼 나열돼 불필요한 입력을 유도 → 주석 처리, `RAG_POSTGRES_HOST`만 필수로 남김. (2) `RAG_GUIDE.md`에 "이미 실행 중이던 앱에 RAG를 나중에 켜면 nginx 재시작 필요" 안내 누락 → 추가. (3) 프로필 청킹 시 frontmatter(`education`/`skills` 등)가 그대로 청크로 잘려 "근거 발췌문"에 노출되던 문제 → `python-frontmatter`로 body만 분리해 청킹. (4) `PROFILE_TOP_K` 5→3 — 프로필이 짧으면 사실상 전체가 다 나올 만큼 과다 노출(Python 기준 5개/4951자 → 3개/2855자). (5) RAG 채팅 패널이 고정 480px 박스라 창 크기를 못 씀 → nav 실제 높이(모바일 줄바꿈 대응)를 JS로 측정해 뷰포트 하단까지 채우고 메시지만 내부 스크롤되도록 수정. `fix/rag-setup-improvements` 브랜치에서 dev 실측 검증(재색인 후 frontmatter 미노출·분량 감소 확인, Playwright로 데스크톱/모바일 레이아웃 확인) 후 `main`에 merge 완료(2026-08-18, `153d0ed`, v1.4.4). merge 직후 prod에서도 재색인 실행, 동일 결과 확인.
- ✅ 프로필 추가 설명 서버 저장 — 업로드 폼의 "추가 설명"이 브라우저 localStorage에만 저장돼 다른 기기·브라우저나 캐시 삭제 시 사라지던 문제(사용자 제보, 2026-08-18). `eval_criteria`와 같은 패턴(`data/candidate_note.md`, 원자적 쓰기)으로 서버 저장하도록 변경 — 업로드 시 자동 저장(LLM 프롬프트용 이스케이프 전 원본), `GET /api/profile/note`로 조회, 프론트는 localStorage 대신 서버에서 불러와 textarea 채움, 백업 ZIP에도 포함. `feat/persist-profile-extra-note` 브랜치에서 저장·조회·초기화 실측 + Playwright로 설정 화면 실제 반영 확인 후 `main`에 merge 완료(2026-08-18, `fbf71ee`, v1.4.5).
- ✅ 업로드 진행 표시 UI 수정 + LLM 응답 잘림 자동 재시도 — 실사용 중 프로필 본문 생성이 `max_tokens`(8192)에 걸려 중간에 잘린 채 저장된 사례 발견(사용자 제보, 2026-08-18). 로그엔 경고가 남았지만 화면엔 안 보여서 눈치채기 전엔 알 방법이 없었음. Claude/OpenAI/Gemini 3개 provider 전부 응답이 잘렸음을 감지하면 `max_tokens`를 2배로 늘려 한 번만 재시도(32768 상한) — `max_tokens`는 실제 상한일 뿐 생성된 토큰만큼만 과금돼서 기본값을 넉넉히 잡는 것 자체는 추가 비용이 없음, 재시도가 실제로 발동하는 드문 경우에만 비용이 두 배로 듦. 프로필 업로드 기본 `max_tokens`도 8192→16384로 상향(재시도 상한 32768과 자연스럽게 맞물림). 겸사겸사 업로드 진행 표시 박스가 `flex-direction: column`이라 텍스트·로딩 스피너가 세로로 쌓이던 UI 버그도 같이 수정. mock 응답으로 재시도 시퀀스 검증 후 `main`에 merge 완료(2026-08-18, `3e0d33d`, v1.4.6).
- ✅ 전체 재분석 후 화면 새로고침 버그 수정 — 실사용 중 "전체 재분석"이 성공했는데도 "재분석 실패"로 잘못 표시되고 화면도 안 갱신되던 문제 발견(사용자 제보, 2026-08-18). `refillCompany()`가 성공 후 `loadDetail(currentSlug)`를 호출했는데 이 함수는 프로젝트 히스토리 전체에 정의된 적이 없었음(`git log -S`로 확인, v0.16 `e45019c`부터 있었던 오타로 추정) — `ReferenceError`가 refill 실패를 잡던 `catch`에 같이 걸려 오분류됨. `initDetail(currentSlug)`(바로 옆 `refitCompany()`가 쓰던 정상 함수)로 교체, `await` 없이 호출해 화면 새로고침 실패가 "재분석 실패"로 오분류 안 되게 함. 실제 회사로 전체 재분석 실행해 콘솔 에러 없음·평가 이력 정상 증가 확인, 사용자도 별도 회사로 직접 확인 완료. `main`에 merge 완료(2026-08-18, `363ade5`, v1.4.7).
- ✅ Codex 리뷰(v1.4.4~v1.4.7 범위) 발견 3건 수정 — 중간 2건·낮음 1건 전부 인정. (1) `llm/gemini.py`: `finish_reason` 잘림 판정이 `usage_metadata` 존재 여부 안에 갇혀 있어서 `usage_metadata=None` 응답(SDK상 정상 발생 가능)에서 `MAX_TOKENS` 잘림을 놓치던 문제 → 판정을 usage 조건 밖으로 분리. (2) `routers/profile.py`: `write_candidate_note()` 실패가 예외 처리 없이 올라가서 이미 성공한 프로필 저장이 500으로 뒤집히고 스냅샷·RAG 재색인 훅까지 건너뛰던 문제 → `OSError`만 좁게 잡아 경고 로그만 남기고 핵심 흐름 계속 진행(`_snapshot_profile()`과 동일 원칙을 놓쳤던 것). (3) `RAG_GUIDE.md`: v1.4.3 이전부터 `RAG_INCLUDE_PROFILE=true`를 쓰던 기존 사용자는 업그레이드만으로 frontmatter 제외가 적용 안 된다는 안내 누락 → 추가. mock으로 Gemini usage 없는 케이스 재시도 확인, Codex와 동일 방식(디렉터리로 만들어 `OSError` 유발)으로 재현 후 예외 처리 확인. `main`에 merge 완료(2026-08-19, `60cf3c6`, v1.4.8).
- ✅ QnA 응답 실패 시 히스토리 롤백 안 되던 문제 — 실사용 중 발견(사용자 제보, 2026-08-19). QnA 질문이 너무 길어 응답이 실패한 경우, 질문은 이미 `qaHistory`(localStorage)에 저장된 뒤라 답 없는 질문이 그대로 남고 다음 질문부터 대화 문맥이 끊긴 채로 이어지던 문제 — 원래는 "프로필 재분석 후 QnA가 예전 대화를 기억 못 한다"는 제보로 시작했으나, 실제로는 히스토리 전달 자체는 정상(devtools로 localStorage 직접 확인)이었고 과거 응답 실패 턴이 조용히 유실되며 user-user가 연속으로 남는 게 원인이었음. `sendQA()`/`sendCompareQA()` 모두 응답 실패 시 방금 push한 질문을 롤백하도록 수정. Playwright로 강제 실패 유발 후 히스토리 길이 불변 확인(수정 전 코드로 동일 테스트 재현 시 실제로 orphan 항목 남는 것도 확인). `fix/qa-history-rollback` 브랜치에서 `main`에 merge 완료(2026-08-19, v1.4.9). 후속 조사로 원래 제보("재분석 후 QnA가 이전 대화를 기억 못함")의 진짜 원인은 이게 아니라 QnA 컨텍스트가 "지금" 스냅샷만 담고 시점별 점수 변화(`fit_history`)를 전혀 안 담는다는 것이었음을 확인 — 대화 히스토리 텍스트에 이전 점수(38점)가 실제로 언급된 적 없음을 직접 확인해 "정보 자체가 없었다"를 검증. `fit_history` 요약을 QnA 컨텍스트에 추가(단일·다중 비교 QnA 모두), prod 실데이터(현대캐피탈, 38→68점 +30)로 확인. 같은 브랜치에 이어서 커밋 후 `main`에 merge 완료(2026-08-20, v1.4.10). 이 조사 도중 프로필의 "이 경험은 점수 산정에 사용하지 말라"는 메모가 적합도 평가에서 무시되는 별개 버그도 발견(같은 공고를 원티드/리멤버 두 플랫폼에 등록해 비교하다가 포착) — 우대사항 충족 근거로 그대로 쓰이고 지원 서류에 기재까지 권장하던 문제. 시스템 프롬프트 [중요] 문단에 한 문장 추가해 수정, dev에서 prod 실데이터(같은 공고+실제 프로필)로 재평가 검증(✅→🔲 다운그레이드, 점수 미반영 주석 확인, QnA는 영향 없이 정상 활용). `fix/fit-eval-exclusion-note` 브랜치에서 `main`에 merge 완료(2026-08-20, v1.4.11). v1.4.11의 프롬프트 지시 방식이 prod 실사용에서 재현 안 됨(같은 프롬프트로 dev 성공·prod 2건 연속 실패, `temperature=0.5`라 지시 준수가 매 호출 확률적) — LLM 판단에 기대는 방식을 폐기하고, 프로필 "추가 설명"의 `[점수 제외]` 섹션을 프로필 생성 LLM에도 안 보여주고 코드로만 본문 뒤에 붙인 뒤(내부 마커), 적합도 평가 프롬프트 조립 시 그 구간을 코드로 통째로 제거하는 방식으로 재설계(`backend/storage.py`). dev에서 가짜 PDF로 전체 파이프라인 실측 검증(업로드→마커 부착→적합도 평가 3회 반복 0건 유출→QnA 정상 활용 확인) 후 prod 실데이터(같은 크몽 공고 2건, 프로필 재생성)로 최종 재확인 — 완전히 0건. 곁들여 다크모드 텍스트 하드코딩 색상, 입력창 크기, placeholder 안내도 함께 수정. `fix/score-excluded-marker` 브랜치에서 `main`에 merge 완료(2026-08-21, v1.4.12).
- ✅ Anthropic 크레딧 소진 시 에러 메시지 불명확 — 실사용 중 발견(2026-08-21). 크레딧 소진 시 Anthropic이 `400`(본문에 "credit balance" 문구)을 반환하는데, `RateLimitError`(429)만 따로 처리하고 나머지 `APIStatusError`는 전부 "잠시 후 다시 시도해주세요"로 뭉뚱그려서 실제 원인(빌링)이 화면에 안 보이던 문제. 에러 본문에서 "credit balance" 문구를 확인해 크레딧 소진이면 402로 명확한 안내를 보여주도록 `backend/llm/anthropic.py`에 헬퍼 추가. 실제로 크레딧이 소진된 상태에서 dev·prod 양쪽 실제 API 호출로 검증. `fix/credit-exhausted-message` 브랜치에서 `main`에 merge 완료(2026-08-21, v1.4.13).
- ✅ RAG 질문 전송 직후 다른 화면 갔다오면 사라지던 문제 — 실사용 중 발견(2026-08-21). `ragCleanupPendingMessages()`가 RAG 화면 진입마다 응답 대기 중(`pending`) 메시지를 무조건 좀비로 간주해 지우던 문제 — "새로고침으로 요청이 끊긴 진짜 좀비"와 "방금 보내서 정상 대기 중인 것"을 구분 안 함. 생성 시각(`createdAt`)을 남겨 2분 이상 지난 것만 정리하도록 수정. Playwright로 5초 전(생존)·3분 전(정리) 두 케이스 검증. `fix/rag-pending-cleanup` 브랜치에서 `main`에 merge 완료(2026-08-21, v1.4.14).
- ✅ Codex 리뷰(v1.4.12~v1.4.14 범위) 발견 3건 수정 — 중간 2건·낮음 1건 전부 재현 확인 후 인정. (1) `storage.py`: `[점수 제외]` 마커를 `re.search()`로 첫 번째만 찾아서 중복 작성 시 두 번째부터 새던 문제 → `finditer()`로 전부 처리. 이 과정에서 빈 마커일 때 정규식이 다음 섹션까지 잘못 삼키는 경계 버그도 추가 발견·수정. (2) `app.js`: RAG pending 정리 기준(2분)이 nginx 실제 타임아웃(300초)보다 짧아 정상 요청도 지울 수 있던 문제 → 10분으로 상향. (3) `index.html`: "틀이 채워집니다" 안내 문구가 실제로는 placeholder일 뿐이라 부정확 → 문구 수정. `fix/codex-review-r1-v1.4.14` 브랜치에서 `main`에 merge 완료(2026-08-21, v1.4.15).
- ✅ 재분석 응답 지연 중 다른 화면 이동 시 가짜 에러 뜨던 문제 — 실사용 중 발견(2026-08-21). 저장·전체재분석·재평가·원티드동기화 4곳 모두, API 응답을 기다리는 동안 다른 화면으로 이동하면 `currentSlug`가 `null`로 바뀌어 뒤늦게 `initDetail(null)` → 404 + 가짜 "로딩 실패" 에러 + 강제 대시보드 이동이 발생하던 문제(실제 저장은 정상 완료됨). 각 함수 시작 시점에 slug를 지역 변수로 고정, 화면 갱신은 그 시점과 `currentSlug`가 같을 때만 수행하도록 수정. Playwright로 브라우저 이벤트 루프 안에서 지연시키는 방식으로 레이스 재현 — 옛날 코드는 정확히 재현, 수정 코드는 재현 안 됨 확인. `fix/stale-currentSlug-refresh` 브랜치에서 `main`에 merge 완료(2026-08-21, v1.4.16).
- ✅ 적합도 평가 이력을 항상 펼쳐서 표시 — UI 개선 요청(2026-08-21). "평가 이력 보기" 클릭해야만 열리던 패널을 데이터 로드 시 바로 렌더링해 항상 열려있게 변경, 버튼은 클릭 시 그 위치로 스크롤 이동만 하도록 단순화(기존 스크롤 동작은 유지). Playwright로 페이지 로드만으로 패널 열림·내용 렌더링·버튼 클릭 시 스크롤 이동 모두 확인. `feat/fit-history-always-open` 브랜치에서 `main`에 merge 완료(2026-08-21, v1.4.17).
- ✅ QnA·RAG 대화 기록을 서버(SQLite)에 저장하도록 전환 — 여러 기기(모바일·집 데스크탑·개인 맥북)를 오가며 쓰는데 대화 기록이 브라우저 localStorage에만 있어 기기 간 연속성이 없던 문제(사용자 제보, 2026-08-21). `data/app.db`에 `qa_messages`, `rag_chats`+`rag_messages` 테이블 추가, pending 상태도 서버에 보존(질문 즉시 INSERT, 응답 시 UPDATE). 설계 중 핵심 발견: `navigate()`가 화면 전환 시 SSE 연결을 끊는데 LLM 호출을 요청 코루틴 안에서 직접 돌리면 페이지 이동 시 처리도 같이 끊겨 "떠났다 돌아오면 답변이 채워져 있다"는 목적이 깨짐 → `asyncio.create_task()` 기반 독립 태스크로 LLM 호출 분리(QnA는 SSE 큐 중계, RAG는 단일 응답이라 그냥 await). 컨텍스트도 클라이언트가 보내던 방식 대신 서버가 직접 조회(최근 20턴, done만)하도록 변경. 좀비 pending은 시간 추측 없이 서버 시작 시점 일괄 정리로 확정적 해결. RAG 채팅방 삭제 시 고아 메시지 방지를 위해 `PRAGMA foreign_keys=ON`+`ON DELETE CASCADE` 적용(SQLite는 FK를 나중에 ALTER로 못 넣어 테이블 생성 시점에 결정). localStorage→서버 1회성 마이그레이션(`migrate-qa`/`migrate-chats`)도 기기별 자동·조용히 실행되도록 구현. curl 강제 연결종료·Playwright 실제 브라우저 시나리오(질문 전송 후 화면 이동→복귀) 양쪽으로 QnA·RAG 각각 검증 — 초반 대기시간을 짧게 잡아 "취소된 줄" 여러 번 오판했으나 충분히 기다리면 매번 정상 완료 확인. prod 실사용 데이터로 QnA 마이그레이션 실제 이관까지 확인(`(주)플래티어` 회사 과거 대화 1건). `feat/chat-history-server-storage` 브랜치에서 QnA·RAG 각각 커밋 후 `main`에 merge 완료(2026-08-22, v1.5.0).
- ✅ v1.5.0 후속 버그 2건 수정 — 배포 직후 검토 중 발견(2026-08-22). (1) `migrate-qa` 재호출 시 중복 삽입 가능 — 서버 처리 성공 후 응답만 유실되면 클라이언트가 완료 플래그를 못 세우고 재호출하는데, 서버 쪽 중복 방지가 없어서(RAG의 `migrate-chats`는 `chat_id` 존재 검사로 이미 막고 있었음) QnA만 이력이 중복될 수 있었음 → 슬러그에 이미 메시지 있으면 건너뛰도록 수정, curl로 동일 페이로드 2회 호출해 1차 삽입·2차 스킵(`inserted:0`) 확인. (2) 회사 삭제 시 QnA 대화 기록 미정리 — `delete_company()`가 `fit_history`는 지우면서 `qa_messages`는 안 지워서, slug 재사용(같은 이름 재등록) 시 예전 QnA 대화가 새 회사에 다시 붙는 문제(v1.4.2의 fit_history 버그와 동일 패턴) → `delete_qa_history_for_slug()` 추가해 같이 호출, 실제 삭제 후 DB에서 0건 확인. `fix/qa-migration-dedup`, `fix/qa-history-orphan-on-delete` 두 브랜치에서 각각 `main`에 merge 완료(2026-08-22, v1.5.1).
...
- ✅ MCP도 같은 구조화 저장 계약 적용 — `create_company`가 raw_text/company_data/judge_result를 재전송 없이 workflow_id 캐시로 참조. Codex 리뷰 4라운드(중간 4·낮음 4 전부 수정) 통과, 실사용량 절감폭은 측정 방법 한계로 신뢰 가능한 숫자를 못 냄(재전송 제거 자체는 코드 레벨로 검증). `main`에 v1.8.0으로 병합·배포 완료(2026-09-04). 상세: `docs/mcp-structured-contract/PLAN.md`·`HISTORY.md`(dev worktree, 로컬 전용)
