...
## v1.8.0 — MCP 구조화 저장 계약 적용 + 워크플로 캐싱 (2026-09-04)

REST(웹)이 v1.7.0에서 도입한 "판정 확정 → 정규화 → 그 결과로 보고서 작성" 2단계
구조를 MCP에도 동일하게 적용. `prepare_company_import()`의 평가 스키마를 구버전
단일 스키마에서 1단계 판정 전용(`evaluate_fit_judge`)으로 교체하고, 신규 도구
`prepare_fit_report()`가 클라이언트가 채운 판정을 REST와 동일한
`fit_normalization` 파이프라인으로 재검증·정규화한 뒤 2단계 보고서 프롬프트를
반환한다. `create_company()`는 `base_body`/`report_prose`를 분리 수신하고 저장
직전 방어적 재정규화를 한 번 더 실행하며, 항목 누락·중복(`evaluation_incomplete`)
시 저장을 거부한다.

실사용 중 "공고 1건 등록에 5시간 사용량 16%를 먹는다"는 보고를 받아 조사한 결과,
`raw_text`/`company_data`/`judge_result`를 여러 도구 호출에 걸쳐 클라이언트가
매번 통째로 재전송해야 하는 구조적 중복(8,284자)을 발견 — 서버 메모리
`workflow_id` 캐시(`backend/services/mcp_workflow_cache.py` 신규, TTL·lock·저장
성공 후에만 정리)를 도입해 `create_company`가 평가 경로에서 해당 인자를 아예 안
받고 캐시만 신뢰하도록 개편. Codex 코드 리뷰 4라운드(중간 4·낮음 4 전부 수정)와
self-check 12개 시나리오로 검증. 부수적으로 `rag/reindex_service.py`의 기존 공통
버그(`trigger_background()` 동기 예외 시 재색인 상태 영구 점유, MCP와 무관하게
웹 CRUD 재색인에도 영향)도 함께 수정.

재전송 제거로 인한 실사용량 절감폭은 실측 3회(버즈빌 기준 16%, 세이지 9%, 버즈빌
재시도 21%)를 시도했으나 매번 다른 교란변수(Sonnet 생각 수준 혼재, "5시간
사용량 %"가 그 세션이 아닌 계정 전체 누적치라 동시 작업에 오염, 재스크래핑 시
raw_text 길이 자체가 달라짐)로 신뢰 가능한 숫자를 얻지 못함. dev API 로그를
직접 대조한 결과, 캐싱이 없애는 몫(추정 3,000~4,500토큰)이 전체 흐름 비용(추출→
본문생성→판정→보고서 4단계를 접속한 클라이언트 세션이 직접 생성하는 비용,
수만 토큰)의 5~9%에 불과해 이 방법으로는 애초에 분리 측정이 어렵다는 결론 —
재전송 제거 자체는 코드 레벨(리뷰+self-check)로 검증됐으므로 이를 근거로 채택.
MCP가 서버에서 LLM을 호출하지 않고 4단계 생성을 전부 클라이언트 세션에 위임하는
구조 자체가 비용의 대부분을 차지한다는 것도 이번 조사로 확인 — 왕복 단계
축소·프롬프트 템플릿 축약 등 추가 절감은 다음 라운드 과제로 남김.

## v1.7.2 — 직무 적합도 분석 표 레이아웃 고정 (긴급 패치, 2026-09-03)

`app.js`가 h3 제목에 "충족 현황"이 포함된 표(자격요건·우대사항)에만 고정 열너비
(`fit-check-table`)를 적용해, 같은 `render_requirement_table()`로 만들어지는
"직무 적합도 분석" 표(제목에 "충족 현황"이 없음)만 자동 너비로 렌더링되던 문제
수정 — 조건에 "직무 적합도 분석"도 포함해 세 표 모두 동일 레이아웃을 쓰도록 통일.

후속 수정: 고정 레이아웃 적용 직후 실사용 중 2번째 칸("관련 경험 여부", 7자)이
기존 "충족 여부"(4자) 기준으로 고정해둔 90px 폭을 넘어 오른쪽 칸을 침범하는
현상 발견 — 항목 칸(1번째)은 그대로 두고 2번째 칸만 오른쪽으로 넓혀(데스크톱
90px→130px, 모바일 22.2%→30%, 근거 칸은 그만큼 축소) 해결. 데스크톱·모바일
너비 모두 Playwright 실제 화면으로 검증.

## v1.7.1 — 마크다운 단일 물결표(~) 취소선 오인 수정 (긴급 패치, 2026-09-03)

프론트엔드 마크다운 렌더러(`marked.js`)가 GFM 표준(`~~이중~~`)과 달리 단일 `~`
쌍도 취소선으로 해석해, "2~3개...5~10점"처럼 문장에 `~`가 두 번 나오면 그 사이
전체가 취소선으로 렌더링되고 `~` 문자 자체가 사라지는 버그가 QnA 답변 등에서
재현됨. 기존에 표 셀 텍스트(`fit_normalization._escape_cell()`)에만 있던 이스케이프를
프론트엔드의 마크다운 렌더링 단일 진입점(`parseMarkdown()`)으로 옮겨, 본문을
생성하는 모든 경로(QnA·리포트 산문·프로필 본문 등)에 한 번에 적용되도록 수정.
이미 이스케이프된 텍스트는 이중 이스케이프하지 않도록 방어. 실제 marked.js
라이브러리로 수정 전/후 렌더링 결과를 직접 비교해 검증.
...
- **소프트스킬 결격사유형 오분류 수정**: 실제 MCP 프로토콜로 2단계 검증하던 중, "협업
  능력"·"커뮤니케이션" 같은 소프트 스킬이 "결격사유형 자격 조항"(해외 출장 가능 여부처럼
  직무 역량과 무관한 형식적 조항)으로 잘못 분류돼 이력서에 근거가 없어도 자동으로 충족
  처리되는 버그 발견. `[자격요건 유형 구분]`에 "협업·커뮤니케이션·문제해결·리더십 등
  소프트 스킬은 절대 결격사유형에 해당하지 않는다"는 명시적 배제 규칙 추가. REST 재검증에서
  회귀 확인, 다만 반복 검증 중 이 배제 규칙도 약 1/3 확률로 지켜지지 않는 잔여 결함 확인 —
  프롬프트 문구를 더 추가하는 대신 판정 단계를 코드로 검증하는 구조적 개선을 논의 중.
- **복합 자격요건 판정 규칙 추가**: 위 수정을 입력 고정 방식으로 재검증하던 중, "RDB/
  MongoDB/Elasticsearch"처럼 여러 기술이 묶인 자격요건에서 자격요건 표(부분 충족)·gaps
  (일부만 갭으로 기록)·종합 의견(전부 충족으로 서술)이 서로 모순되는 내부 불일치 버그
  발견. `[복합 자격요건 판정]` 규칙 추가 — "또는"/"및"은 OR/AND로 명확히 해석하고, "/"·","
  처럼 연결 관계가 불명확한 표현은 전체 충족 처리하지 않고 🔲 불명확으로 통일 표시, 표·
  gaps·종합 의견 간 판정 일치를 요구. REST 3회·MCP 자유형 3회 총 6회 검증에서 목표 버그
  재현 안 됨 확인.

`fix/soft-skill-eligibility-misclassification` 브랜치에서 두 수정 모두 진행, Codex 설계
의견 2라운드 거쳐 `main`에 merge 완료. 상세 기록: `docs/fit-eval-consistency/PLAN.md`·
`HISTORY.md`(dev worktree, 로컬 전용)
...
**동일 버전에 슬쩍 합치지 않고 별도 버전으로 태깅한다.**

- **prod 최초 배포 직후 발견된 규모 버그**: `list_companies`가 실제 운영 데이터(116개
  회사)에서 MCP 클라이언트 라이브러리(`httpx-sse`)의 1MB SSE 이벤트 하드 리밋을 초과해
  실패 — dev 테스트 데이터(69개)로는 안 걸렸던 규모 문제. 서버는 정상 응답을 완전히
  전송했음을 raw HTTP로 직접 확인했고, 원인은 클라이언트 라이브러리의
  `DEFAULT_MAX_EVENT_SIZE_BYTES=1MB` 하드코딩(공식 SDK 소스로 설정 불가 확인 — 어떤
  클라이언트를 쓰든 동일하게 걸림). 즉시 prod를 `git revert`로 원복하고,
  `fix/mcp-tool-call-failure` 브랜치에서 목록 응답의 문장 단위 상세 필드(강점/갭/주요업무
  등, 전체 데이터의 70% 차지)를 제외해 해결(상세 조회는 `get_company`로 그대로 가능,
  1MB 제한과 무관하게 원래도 과도했던 응답 설계였음). 별도 Codex 리뷰 후 재배포. 상세 기록:
  `docs/planning/mcp_sse_size_limit.md`(로컬 전용)
...
중간 1건+낮음 2건)와 자체 리뷰(2건)로 총 7건의 버그를 발견해 수정했다(전부 이 브랜치가
main에 처음 merge되기 전에 발견·수정 완료).
...
질문에 답하면서, 후보자 전체 이력에서 근거를 찾는 도구(`assess_all_gaps_summary`)의 결과(예:
전 직장 PlayOn 프로젝트의 AWS 경험)를 출처 구분 없이 그대로 답변에 섞어 넣던 문제(실사용 로그로
원인 확정, 2026-08-21). `backend/rag/postgres/agent.py`의 `AGENT_SYSTEM` 프롬프트에 "근거
출처를 항상 명시하고 질문이 특정 프로젝트로 범위를 좁혔으면 그 밖 근거와 섞지 마라" 지침 추가
(Codex 권장, 청크 메타데이터 개편 대신 가벼운 방향 채택). prod 실제 로그의 원본 도구 결과를
재생해 old/new 프롬프트 대조 검증 — 스코프 질문 3종×2회 반복(일관성) + 범위 안 좁힌 질문
2종으로 회귀 확인, 총 10/10 통과. `fix/rag-evidence-scoping` 브랜치에서 `main`에 merge.

## v1.5.3 — QnA 버블 대화 시각 표시 + 여백 버그 수정 (2026-08-22)

여러 기기 이력이 한 화면에 섞여 나열될 때(마이그레이션 시점 순서라 실제 대화 순서와 다를 수 있음)
언제 나눈 대화인지 구분 안 되던 문제. 각 질문 버블 아래 "MM.DD(요일) HH:MM" 한 줄 표시(서버 저장
시각 기준). markdown 렌더링 `<p>` 태그의 기본 상하 마진이 답변 버블 padding 위에 겹쳐 버블이
불필요하게 부풀어 보이던 문제도 같이 수정. `fix/qa-timestamp-display` 브랜치에서 `main`에 merge.

## v1.5.2 — Codex 리뷰 5라운드 대응, v1.5.1 후속 버그 9건 수정 (2026-08-22)

v1.5.1을 Codex 코드 리뷰에 올려 5라운드에 걸쳐 순차로 발견·수정한 버그 9건(아래 목록 중
"신규 API가 DB 장애를 확인 안 함"·"RAG 마이그레이션 별도 커밋"·"RAG 태스크 취소 전파"
3건은 최초 CHANGELOG 작성 시 누락돼 2026-08-22 코드 주석 정리 작업 중 추가 기록). 한 브랜치
...
- **RAG 채팅방 동시 마이그레이션 시 500 오류** — 같은 RAG 채팅을 동시에 이관하면 존재 확인
  ...
  0(멱등 스킵) 반환.
- **QnA·RAG 신규 API가 DB 장애를 확인 안 함** — `is_healthy()` 가드가 기존 companies/profile
  라우터엔 있었는데 QnA·RAG의 새 엔드포인트들엔 빠져 있어서, DB 장애 시 빈 목록/500으로
  가려지고 503으로 명확히 안 드러남 → 두 라우터 전부에 동일한 `is_healthy()` 가드 추가.
- **RAG 마이그레이션이 방 생성과 메시지 삽입을 별도 커밋으로 처리** — 방만 만들어진 직후나
  메시지 일부만 들어간 뒤 서버가 죽으면, 재시도해도 "이미 있는 방"으로 판정돼 나머지가 영구
  누락됨 → 방 생성+메시지 전체 삽입을 한 트랜잭션으로 묶어 중간 실패 시 전부 롤백, 재시도
  시 처음부터 다시 시도 가능하도록 수정.
- **RAG 응답 생성 태스크가 요청 취소에 함께 취소될 수 있음** — `asyncio.Task.cancel()`은
  "지금 await 중인 Future/Task"에도 취소를 전파하는 문서화된 동작이라, 클라이언트 연결이
  끊기면(`await task`로 기다리던) 독립 태스크까지 같이 취소돼 pending으로 남을 수 있었음 →
  `asyncio.shield(task)`로 감싸 바깥 취소가 태스크 자체엔 전파 안 되도록 방어.
- **QnA SSE 연결이 끊기면 같은 질문으로 자동 재시도해 이력·비용이 중복됨** — localStorage
  시절엔 재시도가 안전했지만, 서버 저장 전환 후엔 POST 한 번마다 새 행+새 LLM 호출이 생겨서
  재시도가 그대로 중복 생성으로 이어짐 → 재시도 로직 제거, 실패 시 호출부가 서버 상태를
  다시 조회하도록 변경. `consumeSSE()`도 `[DONE]`을 못 보고 끝나면 부분 텍스트 대신 `null`을
  반환하도록 수정(부분 텍스트를 그대로 반환하면 `sendCompareQA()`가 잘린 응답을 성공으로
  착각해 히스토리에 남기던 문제도 같이 해결).
...
- **QnA 마이그레이션 재호출 시 중복 삽입** — `migrate-qa`는 서버가 마이그레이션을 성공적으로
  처리해도, 그 응답이 클라이언트에 못 돌아오면(네트워크 유실 등) 완료 플래그를 못 세우고 다음
  로드 때 같은 데이터로 재호출할 수 있는 구조였는데, 서버 쪽에 중복 방지가 전혀 없어서
  이 경우 QnA 이력이 중복 삽입될 수 있었음. RAG의 `migrate-chats`는 `chat_id` 존재 여부로
  이미 막고 있었는데 QnA에는 이 검사가 빠져있었음 — 슬러그에 이미 메시지가 있으면 그 슬러그는
  건너뛰도록 수정. curl로 같은 페이로드를 연속 두 번 보내 1차엔 정상 삽입, 2차엔
  `inserted:0`으로 건너뛰어짐을 확인.
- **회사 삭제 시 QnA 대화 기록 미정리** — `delete_company()`가 평가 이력(`fit_history`)은
  지우면서 QnA 대화 기록(`qa_messages`)은 안 지우고 있었음. slug는 회사명+직무명으로 결정적
  생성되므로, 같은 이름으로 재등록하면 예전(별개) 지원의 QnA 대화가 새 회사에 그대로
  다시 붙는 문제 — `fit_history`가 v1.4.2에서 이미 겪었던 것과 같은 패턴을 QnA 서버 저장
  전환 때 놓쳤음. `delete_qa_history_for_slug()` 추가, 회사 삭제 시 같이 호출하도록 수정.
  실제 QnA 대화가 있는 회사를 삭제해 DB에서 직접 0건 확인.
