...
## v1.4.18 — QnA 대화 기록을 서버(SQLite)에 저장하도록 전환 (2026-08-22)

여러 기기(모바일·집 데스크탑·개인 맥북)를 오가며 쓰는데 회사별 QnA 대화 기록이 브라우저
localStorage에만 있어 기기 간 연속성이 없던 문제. `data/app.db`(프로필 히스토리와 같은
SQLite 파일)에 `qa_messages` 테이블을 추가해 서버를 단일 진실 공급원으로 전환(RAG 채팅은
아직 미착수, 다음 버전에서 별도 진행 — `docs/chat-history-server-storage/`).
...
- **LLM 호출을 HTTP 요청 생명주기와 분리**: `frontend/app.js`의 `navigate()`가 화면 전환 시
  ...
  깨짐. `asyncio.create_task()` 기반 독립 태스크로 LLM 호출을 분리하고 SSE는 큐를 중계만
  하도록 변경 — 클라이언트 연결과 무관하게 끝까지 실행됨을 curl 강제 연결종료·실제 Playwright
  브라우저 시나리오(질문 전송 후 화면 이동→대기→복귀) 양쪽으로 검증.
- **컨텍스트 조립 방식 변경**: 클라이언트가 매 요청 `history`를 보내던 방식 대신, 서버가
  `qa_messages`에서 직접 최근 20턴(`status='done'`만)을 조회해 조립 — 서버가 진실 공급원이
  되는 취지와 일치.
...
- **1회성 마이그레이션**: `POST /api/companies/migrate-qa`로 기기별 localStorage의 옛 QnA
  기록을 서버로 이관. 각 기기에서 앱을 한 번 열면(`migrateQAHistoryIfNeeded()`) 자동·조용히
  실행, 실패 시 다음 로드 때 재시도. user/assistant 짝이 안 맞는 마지막 항목(과거 버그로
  응답 없이 남은 질문)은 건너뜀.
