...
> **v1.4.3 이전부터 `RAG_INCLUDE_PROFILE=true`를 이미 켜서 쓰고 계셨다면**: v1.4.4에서 프로필 청킹 시 frontmatter(스킬 목록 등 구조화 필드)를 제외하도록 개선됐는데, 이건 업그레이드만으로는 자동 적용되지 않습니다. 데이터가 깨지진 않지만(기존 청크·벡터는 그대로 서로 맞는 상태), 개선된 결과를 보려면 "🔄 재색인"을 한 번 더 눌러주세요.
