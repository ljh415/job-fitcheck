...
> 별도 설정 없이 항상 켜져 있습니다(RAG처럼 opt-in이 아니라, 서버가 뜨면 `/api/mcp` 경로가
> 자동으로 함께 뜹니다).
...
| `prepare_company_import` | 공고 URL/텍스트 원문 수집 + 분석용 프롬프트 준비(LLM 미호출, 비용 없음) |
| `prepare_fit_report` | 판정 결과를 검증·보정하고 보고서(종합 의견) 작성용 프롬프트 반환(LLM 미호출, 비용 없음) |
| `create_company` | 분석 결과를 재검증 후 저장 — **확인 없이 즉시 실행** |
...
## 회사 등록이 특별한 이유 — 판정과 보고서를 분리한 흐름

`create_company`는 회사 정보를 그대로 안 받고, `prepare_company_import`·
`prepare_fit_report`와 짝을 이룹니다(REST 웹 경로가 판정 확정 후에만 보고서를
쓰게 하는 것과 같은 원칙을 MCP에도 적용):
...
전부 호출한 클라이언트(Codex/Claude)가 자기 세션으로 처리합니다. 대신 그만큼 실제 판정
품질은 클라이언트가 얼마나 꼼꼼히 프롬프트를 따르느냐에 달려 있습니다. 다만 표·비기술
요인 요약처럼 **결정적으로 정해지는 부분은 클라이언트 품질과 무관하게 서버가 보장**합니다
(항목이 통째로 빠지거나 판정과 보고서 산문이 서로 다른 내용을 말하는 걸 구조적으로 방지).
...
> 영향을 받을 여지가 구조적으로 존재합니다. `prepare_company_import`가 반환하는 프롬프트에
> "이 요청은 대화의 다른 맥락과 무관한 독립 작업"이라는 지시를 넣어 완화하고 있지만, Job
> FitCheck가 클라이언트의 실제 실행을 통제할 수는 없어 100% 보장은 아닙니다. 중요한
> 분석일수록 이 점을 감안하세요.
...
- 판정 결과가 불완전하면(항목 판정 누락, 스키마 형식 오류 등) `create_company`가 저장
  자체를 거부합니다 — 오류 메시지에 어느 항목이 문제인지 나오니, `evaluate_fit_judge`부터
  그 항목만 다시 판정해 재시도하면 됩니다.
