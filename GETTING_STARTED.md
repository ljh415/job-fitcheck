...
> **Mac에서 "확인되지 않은 개발자"라며 안 열리면?** 애플 공식 서명이 없는 일반 스크립트라 뜨는 정상적인 경고입니다. macOS 최신 버전에서는 더블클릭만으로 우회가 안 되고 아래 순서를 거쳐야 합니다.
> 1. 경고 창에서 **확인**을 눌러 닫기
> 2. 애플 메뉴 → **시스템 설정** → **개인정보 보호 및 보안**으로 이동
> 3. 아래로 스크롤하면 방금 막힌 `start-uv.command`에 대한 안내와 함께 버튼이 보입니다 — **"그래도 열기"**(비슷한 문구일 수 있음)를 클릭
> 4. 로그인 비밀번호를 입력하면 허용되고, 그다음부터는 다시 묻지 않습니다
>
> 이 버튼은 파일을 한 번 열어보려고 시도한 뒤 **약 1시간 동안만** 나타납니다 — 시스템 설정에 버튼이 안 보이면 `start-uv.command`를 다시 더블클릭한 뒤 설정 화면으로 돌아가보세요.
