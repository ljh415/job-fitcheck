...
> ### 💡 Docker 설치가 부담스럽다면: `uv`로 더 가볍게 실행하기 (선택)
>
> Docker Desktop은 용량이 크고 백그라운드에서 계속 실행되는 프로그램입니다. 컴퓨터를 다뤄본 경험이 있거나 가볍게 실행하고 싶다면, `uv`라는 훨씬 작은 도구로도 이 앱을 실행할 수 있습니다.
>
> - **이 경우 1단계(Docker Desktop 설치)는 건너뛰세요.** 2단계(프로젝트 다운로드)·3단계(API 키 발급)는 그대로 진행합니다.
> - 4단계(초기 설정)도 동일합니다 — `run/setup.command`/`run/setup.bat`을 그대로 실행하세요.
> - 5단계(앱 실행)에서만 `run/start.command`/`run/start.bat` 대신 **`run/start-uv.command`(Mac)** 또는 **`run/start-uv.bat`(Windows)**를 더블클릭하세요.
>   - `uv`가 설치돼 있지 않으면 "지금 설치할까요? (Y/n)"라고 물어봅니다 — 그냥 Enter만 누르면 자동으로 설치되고 이어서 실행됩니다.
>   - 처음 실행할 때는 Python과 필요한 패키지를 받느라 수 초~1분 정도 걸릴 수 있습니다.
>   - 끌 때는 이 창에서 `Control + C`를 누르거나 창을 그냥 닫으면 됩니다(Docker와 달리 별도의 `stop` 스크립트가 필요 없습니다).
> - 6단계(접속 및 로그인)부터는 이후 내용과 동일합니다.
>
...
> `uv`로 실행하셨다면 `stop` 스크립트 없이 그냥 창을 닫으면 꺼지고, 다시 켤 때는 `start-uv.command`/`start-uv.bat`을 다시 더블클릭하면 됩니다.
...
| `port is already allocated` 같은 오류 | 8000번 포트를 다른 프로그램이 이미 쓰고 있습니다. 어떤 프로그램인지 찾을 필요 없이 포트 번호를 바꿔서 우회하면 됩니다: 압축 푼 폴더의 `docker-compose.yml` 파일을 텍스트 편집기(Mac: `open -e docker-compose.yml` / Windows: `notepad docker-compose.yml`)로 열어 `"8000:80"` 부분을 `"8080:80"`처럼 다른 숫자로 바꾸고 저장 → `run/start.command`/`start.bat`을 다시 실행 → 브라우저 주소도 바꾼 숫자로(`http://localhost:8080`) 접속 |
| 로그인이 안 됨 | 4단계에서 정한 비밀번호를 정확히 입력했는지 확인 (대소문자 구분, 키보드 한/영 전환 상태도 확인 — 한글 입력 모드에서 영문 비밀번호를 치면 다른 문자로 입력됩니다) |
