...
- **적합도 평가** — 이력서 대비 0~100점, 커스텀 기준 지원. 판정(1단계)과 보고서 산문(2단계)을 분리해 표·점수는 코드가 결정론적으로 생성
- **Q&A 채팅** — 공고 기반 면접 준비 질문 응답
- **대시보드** — 필터·검색·핀·비교 테이블, 탈락 공고 숨기기
- **지원 현황 관리** — 타임라인·캘린더 뷰, 8가지 상태
- **프로필 히스토리 & 평가 이력 추적** — 이력서를 새로 올리거나 수정할 때마다 스냅샷 보관, 회사별 적합도 평가는 덮어쓰지 않고 이력으로 누적 — 프로필이 좋아지면서 점수가 어떻게 바뀌었는지 설정 화면·공고 상세 페이지에서 확인 가능
- **내보내기** — ZIP·CSV·자동 백업
- **로컬 저장** — 회사 정보는 마크다운 파일, 별도 DB 서버 없이 데이터 로컬 유지
...
| 프로필 추출·적합도 평가·Q&A | claude-sonnet-5 | gpt-5 | gemini-3.5-flash |
