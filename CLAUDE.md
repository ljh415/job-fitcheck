...
cp .env.example backend/.env  # 동일한 키를 backend/.env에도 복사
...
pydantic-settings는 `backend/.env`를 직접 읽고, Docker Compose는 루트 `.env`를 읽습니다.
**두 파일의 API 키가 다르면 인증 오류(401)가 발생합니다.**

```
루트 .env           → Docker Compose가 컨테이너에 환경변수로 주입
backend/.env        → 로컬 직접 실행 시 pydantic-settings가 읽음
```

키를 변경했다면 두 파일 모두 업데이트하세요.
...
- **LLM 티어**: Lightweight(Haiku) = 구조화 추출·요약, High(Sonnet) = 프로필 추출·적합도 평가·Q&A. 설정 화면에서 모델 수동 변경 가능.
